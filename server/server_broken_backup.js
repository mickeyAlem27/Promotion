...
const setupSocketIO = (server) => {
  ...
  io.on('connection', (socket) => {
    ...
    // Handle user heartbeat for keeping connection alive
    socket.on('user_heartbeat', (data) => {
      console.log(`💓 Heartbeat received from user ${data.userId}`);

      // Update user's last seen timestamp if user is authenticated
      if (data.userId && socket.userId === data.userId) {
        // User is still active, update their status
    ...
    // Handle user room rejoin after reconnection
    socket.on('rejoin_user_room', (data) => {
      if (data.userId) {
        socket.join(`user_${data.userId}`);
        console.log(`🔗 User ${data.userId} rejoined their personal room`);
      }
    });
    ...
    socket.on('send_notification', async (data) => {
      ...
      try {
        // Save notification to database if MongoDB is connected
        if (mongoose.connection.readyState === 1 && data.targetUserId) {
        ...
        // Broadcast notification to all connected users or specific user
    ...
    socket.on('mark_notification_read', async (notificationId) => {
      ...
      try {
        // Update notification in database if MongoDB is connected
        if (mongoose.connection.readyState === 1) {
          const Notification = require('./models/Notification');

          // Check if notificationId is a database ID or a generated ID
          if (notificationId.startsWith('notif_')) {
            // This is a generated ID, find the actual notification
            const notifications = await Notification.find({
              recipientId: socket.userId
            }).sort({ createdAt: -1 }).limit(10);

            // Find the most recent notification that matches the criteria
            const latestNotification = notifications.find(n => n.read === false);
    ...
    socket.on('get_notifications', async () => {
      try {
        if (mongoose.connection.readyState === 1) {
          ...
          .sort({ createdAt: -1 })
          .limit(20)
          .populate('senderId', 'firstName lastName photo role');
          ...
          socket.emit('notifications_list', {
            notifications: notifications.map(notif => ({
              ...
              sender: notif.senderId ? {
                ...
                firstName: notif.senderId.firstName,
                lastName: notif.senderId.lastName,
                photo: notif.senderId.photo,
                role: notif.senderId.role
              } : null,
              data: notif.data || {}
            }))
          });
        ...
        socket.emit('notifications_error', { error: 'Failed to load notifications' });
      }
    });
  });
